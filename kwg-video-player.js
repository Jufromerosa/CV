...
(function (document, window) {
    ...
    kwgVideo.prototype = {
        ...
        _createVideoElements: function () {
            ...
            if (this.options.containerClass != '') {
                this.videoElements.wrapperVideo.className += ' ' + this.options.containerClass;
            }
            ...
            this.videoElements.progressWrapBarContainer = document.createElement('div');
            this.videoElements.progressWrapBarContainer.className = 'progressWrapBarContainer';
            this.videoElements.videoControls.appendChild(this.videoElements.progressWrapBarContainer);

            this.videoElements.progressBarContainer = document.createElement('div');
            this.videoElements.progressBarContainer.className = 'progressBarContainer';
            this.videoElements.progressWrapBarContainer.appendChild(this.videoElements.progressBarContainer);

            this.videoElements.playProgress = document.createElement('div');
            this.videoElements.playProgress.className = 'playProgress';
            this.videoElements.progressBarContainer.appendChild(this.videoElements.playProgress);
            ...
            this.videoElements.playProgress.appendChild(this.videoElements.playProgressHandle);

            this.videoElements.hoverProgress = document.createElement('div');
            this.videoElements.hoverProgress.className = 'hoverProgress';
            this.videoElements.progressBarContainer.appendChild(this.videoElements.hoverProgress);
            ...
            this.videoElements.progressBarContainer.appendChild(this.videoElements.bufferedProgress);
            ...
            this.videoElements.leftControls.appendChild(this.videoElements.btnPlayPause);
            ...
            this.videoElements.leftControls.appendChild(this.videoElements.volumePanel);

            // volume Slider
            this.videoElements.volumeSlider = document.createElement('div');
            this.videoElements.volumeSlider.className = 'volumeSlider';
            this.videoElements.volumePanel.appendChild(this.videoElements.volumeSlider);

            // sound Volume
            this.videoElements.soundVolume = document.createElement('div');
            this.videoElements.soundVolume.className = 'soundVolume';
            this.showVolume();
            this.videoElements.volumeSlider.appendChild(this.videoElements.soundVolume);
            ...
            this.videoElements.settingsWrap.appendChild(this.videoElements.settingsButton);
            ...
            for (var i = 0; i < speedOptionsLength; i++) {
                this.videoElements.speedMenu.innerHTML += '<li data-speed="' + this.options.speedOptions[i].speed + '">' + this.options.speedOptions[i].speedLabel + '</li>';
            }
            ...
            this.videoElements.settingsWrap.appendChild(this.videoElements.settingsMenu);

            //options settings menu
            this.videoElements.speedOption = document.createElement('li');
            this.videoElements.speedOption.className = 'speedOption';
            this.videoElements.speedOption.innerHTML = '<span class="item-label">Play speed</span><span class="item-content"></span>';
            this.videoElements.settingsMenu.appendChild(this.videoElements.speedOption);

            // repeat option
            this.videoElements.repeatOption = document.createElement('li');
            this.videoElements.repeatOption.innerHTML = '<span class="item-label">Repeat</span><span class="item-content"></span>';
            this.videoElements.settingsMenu.appendChild(this.videoElements.repeatOption);



            // full screen
            this.videoElements.fullscreenButton = document.createElement("div");
            this.videoElements.rightControls.appendChild(this.videoElements.fullscreenButton);
        ...
        _registerEvents: function () {
            ...
            this.videoElements.progressBarContainer.addEventListener('mousemove', this.showHoverProgress.bind(this));
            this.videoElements.progressBarContainer.addEventListener('mouseleave', this.hideHoverProgress.bind(this));
            ...
            this.videoElements.volumeSlider.addEventListener('mousemove', this.showHoverVolume.bind(this));
            this.videoElements.volumeSlider.addEventListener('mouseleave', this.hideHoverVolume.bind(this));
            this.videoElements.volumeSlider.addEventListener('click', this.changeVolume.bind(this));
            this.videoElements.volumePanel.children[0].addEventListener('click', this.toggleMute.bind(this));
        ...
        draggingVolumeHandle: function (event) {
            if (hasClass(this.videoElements.soundVolumeHandle, 'draggingVolumeHandle')) {
                this.changeVolume(event);
            }
        },
        endDraggingVolumeHandle: function () {
            if (hasClass(this.videoElements.soundVolumeHandle, 'draggingVolumeHandle')) {
                removeClass(this.videoElements.soundVolumeHandle, 'draggingVolumeHandle');
            }
        },
        ...
        draggingPlayHandle: function (event) {
            if (hasClass(this.videoElements.playProgressHandle, 'draggingPlayHandle')) {
                this.changeCurrentTime(event);
            }
        },
        endDraggingPlayHandle: function () {
            if (hasClass(this.videoElements.playProgressHandle, 'draggingPlayHandle')) {
                removeClass(this.videoElements.playProgressHandle, 'draggingPlayHandle');
            }
        },
        setKeyboardEvents: function (event) {
            if (this.options.keyboard && hasClass(this.videoElements.wrapperVideo, 'active-p')) {
        ...
        showBufferedProgress: function () {
            if (this.el.duration > 0) {
                for (var i = 0; i < this.el.buffered.length; i++) {
                    if (this.el.buffered.start(this.el.buffered.length - 1 - i) < this.el.currentTime) {
                        this.videoElements.bufferedProgress.style.width = (this.el.buffered.end(this.el.buffered.length - 1 - i) / this.el.duration) * 100 + "%";
                        break;
                    }
                }
            }
        ...
        setSpeedLabel: function () {
            ...
            for (var i = 0; i < speedLislength; i++) {
                if (this.el.playbackRate == speedLis[i].getAttribute('data-speed')) {
                    this.videoElements.speedOption.getElementsByClassName('item-content')[0].innerHTML = speedLis[i].innerHTML;
    ...
    function addClass(el, className) {
        ...
        arrayClasses = arrayClasses.filter(function(value, index){ return (arrayClasses.indexOf(value) == index && value != '') });
